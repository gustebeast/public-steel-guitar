...
PARTS = {
    ...
    "leg_trrs_throat": (lambda: heal(__import__("src.leg_trrs", fromlist=["e"]).throat()), "pctg/leg_trrs_throat.step", "PCTG — the TRRS jack's up-stop at the fixed tenon's tip: a Ø10.6 × 6.2 ring pressed in LAST, bored Ø6.6 so the body plug's overmould still passes, with a 45° funnel at the mouth that raises the plug's radial capture from 1.55 to 2.75. It cannot be a step in the bore — the Ø9.7 jack goes in from the tip, so a lip above it would be a lid fitted before the box is filled. A groove in its OD takes leg_trrs_pin. Carries the coil's 5 N only while the leg is off; with the leg on the mortise roof sits over it. Prints flat"),
    "leg_trrs_pin":    (lambda: heal(__import__("src.leg_trrs", fromlist=["e"]).pin()), "tpu/leg_trrs_pin.step", "TPU — the keeper's lock pin ×4 (Ø2.2 × 5.0, one per leg): pushed into the fixed tenon's -X flank — the near flank, 9.8 from the spine where -Y is 21 — and into a groove in leg_trrs_throat's OD. Blind, so SINGLE shear: the coil's 5 N preload on Ø2 is 1.6 MPa against TPU's ~12, i.e. 7.5×, and it is unloaded entirely whenever the leg is ON. PRINTED, not a dowel (user): TPU's give is what makes a printed pin in a printed hole a fit (PIN_FIT 0.2 of squeeze) rather than a gamble. Both ends are shaped, not flat — the inner saddled to the groove's root, the outer trimmed to the sloping octagon flank, since that flank meets the mortise with 0.15 of clearance and a proud pin would jam the leg. It replaces an M4×16 button and its brass insert, and the head burial they needed — 2.4 mm of keeper height — is handed to the male plug's wrap, which is what keeps it pointing straight with the leg off. Solid — a pick dimple would have left a 0.70 wall; drive a scribe in to pull it. Prints upright"),
...
_COLORS = {
   ...
   # steel dowel (gauged break pin)
    ...
    "leg_trrs_pin":    (0.03, 0.03, 0.03),   # ...which a printed TPU pin locks (black)
