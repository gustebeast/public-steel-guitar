...
PARTS = {
    ...
    "leg_shaft":       (lambda: heal(LG.leg_shaft()),   "petg-gf/leg_shaft.step",   "PETG-GF — -Y shaft ×2: W28 dropped-octagon tenon (197, rides under the sleeve cover) + SOLID block (91, equal-height rule; +Y face truncated to the 17.8 stem plane = one flat print bed) with the shared TPU foot's dovetail mortise. 288 long: print LYING on that face, laid diagonal in plan"),
    ...
    "leg_shaft_short": (lambda: heal(LG.leg_shaft_short()), "petg-gf/leg_shaft_short.step", "PETG-GF — SHORT +Y shaft ×1 (the wired one prints from leg_shaft_trrs): W28 dropped-octagon tenon ending in the terminal block (+Y face truncated to the 17.8 stem plane = one flat print bed) whose octagon socket + point-side ledge take the bar tower's matching shaved tenon"),
