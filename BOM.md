...
| **M4 × 18 button screw** | M4 × 0.7, 18 mm, 18-8 SS button head (ISO 7380, 2.5 mm hex) | 4 | [McMaster 92095A-series](https://www.mcmaster.com/92095A192/) | ~$12 / pack [m] | THE LEG'S ONE SCREW, one per corner (`legs.lock_pin_joint`). NEW LENGTH 2026-09-17, with the straight end-wall TONGUE deleted (user): the screw threads into a heat-set insert in the ENDPLATE -- in the material that tongue's rebate used to take out of it -- and carries on through a Ø4.4 clearance hole in the chassis floor into the foot's OUTERMOST TENON, which it pins. That is the whole leg retention: the tenon cannot slide in Y past the pin, and the same screw holds the endplate to the chassis. It was M4 × 12 into an insert in the CHASSIS, which forced the chassis to keep a mortise-free zone for the pocket -- and once the mortise grid ran end to end, that pocket broke into a mortise anyway (97 mm3 of it). Tip lands 4.84 into the tenon, asserted in legs.py against the real grid. **Confirm the ×18 length suffix at purchase.** |
...
| **Tee PCB** | custom 49.5 × 16 L (40 × 16 layout + a bare 9.5 × 8.7 mounting ear, M4 through-hole): 3× B4B-XH-A + 120 Ω + shunt jumper | 12 | ~$2 assembled (est.) | JLCPCB | panelized with the sensor boards; close the jumper on the LAST tee = bus-B termination |
