...
| **M2 clamp screw** | M2 × 0.4 socket cap, 8 mm | 10 | [McMaster](https://www.mcmaster.com/) | commodity | The screw pulleys' TORQUE PATH — they had none at all, just a plain Ø5 bore on a round rod. Each pulley is now a **C-clamp**: one full-height slit and this screw squeezing the bore onto the thread. **A grub screw was the first design and was rejected (user):** a set screw bearing on a single thread crest is a point contact that depends on preload staying put, while a clamp grips the whole circumference and cannot back off into a valley. Two further constraints fixed where it goes, both found by the swept check: it must sit **above the belt** (anything at r > the tooth OD inside the toothed band jams the belt once per turn), and its axis must be **X** (along Y a driver would have to reach past every other station in the row). So it lives in a hub stacked on the top flange, at the flange Ø so it adds nothing to the swept circle. Self-tapped into the far ear — an M2 insert pocket is 3.5 deep and the ear is 3.4 long, and this is a clamp screw, not a set screw bearing on the shaft. 0.072 N·m of drive torque is only 29 N at the rod surface, so M2 is ample |
| **M2 nut screw** | M2 × 0.4 socket cap, 10 mm | 20 | [McMaster](https://www.mcmaster.com/) | commodity | Two per station, up through the H-nut's ears into the carriage — this is what carries the string pull from carriage to nut, 73.5 N each in tension (~2.4 MPa on the self-tapped thread over a 4.8 mm bite). Length spans the 3.2 flange + the bite; **re-check it against the real flange thickness when the nut arrives** |
