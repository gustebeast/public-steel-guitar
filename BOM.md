...
| **TRRS coil mandrel** (printed, 2 pc) | `tools/coil_mandrel.step` + `tools/coil_mandrel_sleeve.step` — wind the leg's TRRS lead 7 turns on the Ø16.0 barrel, slide the Ø23.8-bore sleeve over it, heat-set | 1 set | PRINT IT | filament only | **Why a tool and not an instruction.** The leg's slack store is a coil in the gap between the two tenon ends, and the TURN COUNT decides how low the leg can go: the coil gets fatter as it compresses, and below ~6.59 turns it swells past the sleeve's Ø24.7 bore before the leg reaches its low stop. At 6.5 turns it binds at a 71.3 mm gap against a 90.8 minimum; at 7 it binds at 26.6, about eleven ladder steps of headroom. Half a turn is the difference, so the count is fixed by a scribe line rather than by counting. **Why TWO pieces (user).** A bare mandrel sets only the coil's inside diameter and leaves the outside to spring-back — a number nobody can quote for an unspecified jacket, against a bore that allows only 20.9 of mean. Capturing the cable in an annulus its own width pins the mean between two surfaces: wound on 16.0, capped at 23.8, mean 19.8 by arithmetic. The outer also holds every turn put through the heat and the cool. **⚠ Heat is not optional** — winding cold and leaving it gives creep, not a set. ~80 °C water, 15–20 min, cool fully BEFORE opening (the set happens on cooling). **⚠ And PETG-GF's Tg is ~80 °C**, the bottom of the useful range: print both SOLID, keep the water at 80 rather than boiling, treat them as consumable, or substitute a metal tube of Ø16.0 for the inner — the sleeve's job is unaffected. **Only needed if a pre-coiled TRRS lead with a real datasheet cannot be bought** (see docs/leg-trrs-routing.md) |
