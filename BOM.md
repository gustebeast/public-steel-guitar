...
| **TRRS M-M patch** (adapter → chassis socket) | 3.5 mm 4-pole M-M, ~15 cm, shielded; **pick at purchase**: moulded plug handle ≤ Ø10 both ends | 1 | ~$4 | (commodity) | the SHORT hop the leg's signal makes OUTSIDE the shell: one end is captive in the body adapter's bore (spring-floated, `leg_trrs`), the other plugs into the adapter board's socket in the +Y rail. It leaves the adapter's SIDE, not its top — the leg slides along Y to come off, and a cable out of the top face is one the leg shears |
