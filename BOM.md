...
| **M4 heat-set insert** | M4 × 0.7 brass heat-set, 4.7 mm | 44 | [McMaster 94459A150](https://www.mcmaster.com/94459A150/) | $10.82 / pack 50 [m] | 10 nut clamps + 4 leg-sleeve pinch collars + 3 pickup-carrier jack nuts + 1 pickup -Y retention grub; deeply buried (no pull-out) + **11 CAN-tee side hold-downs** (one per bus-A cradle boss). + **4 leg lock pins** (one per corner, in the chassis's kept shell — `legs.lock_pin_joint`) + **1 pedal-bar latch collar** (in the bar's tower, `bar_latch`). ⚠ **Recount before ordering** — the 10 nut clamps and 4 leg-sleeve pinch collars above are stale (the keyhead uses sliding inserts; legs.py thread-forms the pinch grubs), while the 10 belt-tensioner insert-nuts, the 2 optical-strip grips and the knee/pedal spring-tension inserts are not listed. + **10 keyhead height-screw nuts** (prototype; mouth down, flush in the keyhead prism's bottom face). | + **1 leg TRRS keeper lock** (in the fixed tenon's -Y flank, `leg_trrs.lock_negatives`). ⚠ it is on a **30° TILTED** axis: square to the flank the O7.6 button head stood 1.9 proud of the tenon's tip and would have fouled the mortise roof.
...
| **M4 × 16 button, 2.5 hex** | ISO 7380 M4×16, head Ø7.6 × 2.2 | 1 | stock | — | the leg TRRS keeper's lock: drives into the heat-set insert above and its tip sits `LOCK_GROOVE` deep in the keeper's OD groove. Longer than the M4×10 elsewhere because the axis is tilted and has to cross 18.6 of flank. **Same 2.5 hex as every other headed screw on the instrument** — note a headless M4 set screw would have been a 2.0 hex and a second driver, which is why this is a button in a recess and not a grub |
| **TRRS float spring** | Compression, **ID ≥ 6.5 (O8.0 OD × 0.7 wire × 12 free)**, rate ~2.5 N/mm, solid ≤ 4.9, 302/304 SS | 1 (+ spares) | ⚠ **PART NUMBER OPEN** — envelope specified, supplier not yet pinned (Lee Spring's catalogue will not filter from a URL and its pager is client-side; McMaster is unreadable from here). Buy to the envelope | — | SECOND spring SKU, and it is not a preference: the leg's blind-mate coil has to end up ON the lead, and a lead has two ends — the moulded jack (Ø9.7) and the moulded far plug (Ø6.1). The latch coil's **3.8 ID passes neither**, so no assembly order puts it there and the joint was unbuildable with it (user found this). The ONE number that changes is the ID; free length and rate are the latch coil's, so the feel is the same **5.0 N at rest / 12.5 N seated**. It pays for itself: at Ø8.0 the coil seats on the tenon's own bore step and the printed seat washer it needed at Ø5.0 is gone |
...
| **TRRS M-M patch** (adapter → chassis socket) | Tensility **10-02155**: 3.5 mm 4C plug-to-plug, 1830 mm, 28 AWG shielded. **Overmould Ø6.1 × 14, barrel Ø3.5 × 14, cable Ø3.8** — all four off the drawing, and `leg_trrs` is built to them | 1 | **$3.73** [v] | [DigiKey](https://www.digikey.com/en/products/detail/tensility-international-corp/10-02155/7606602) · [Drawing](https://tensility.s3.us-west-2.amazonaws.com/uploads/pdffiles/10-02155.pdf) | the SHORT hop the leg's signal makes OUTSIDE the shell. One end PRESSES into the body adapter's roof **down the octagon joint's own axis** (user), its barrel hanging into the mortise where the leg's tenon carries the SPRING-FLOATED jack to meet it — so the blind mate is centred by the joint itself and seats wherever the latch stops. The other end plugs UP into the adapter board's jack on the instrument's -Z UNDERSIDE, a hand's reach inboard of the leg, and unplugging THAT is what frees the leg. Between them it leaves the adapter's top face by a straight CHANNEL out the -Y side, so the whole visible run is on the underside. **1830 is far more than the ~150 needed** — coil the excess in the cavity, or ask Tensility for the same assembly short (they build to order) | **The LEG side's M->F extension is a separate line and its CABLE OD is a hard constraint: ≤ 3.2 mm.** The latch coil (Ø5.0, 0.6 wire, 3.8 ID) is dropped down the tenon over that lead once it is threaded, so it has to fall freely over it — a 3.8 lead is a zero-clearance fit that binds. `leg_trrs` asserts it.
