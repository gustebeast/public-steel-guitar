...
| 3 | **Tr5×1 nut** | **(a) The NUT does not fit** — every stocked Tr5×1 nut is the Ø20-flanged 3D-printer type, and the carriage lane is 9.5. **(b) stock is thin** — every length variant shows single/low-double-digit counts, some already unavailable, because this size sits outside the standard supply chain | (a) measure a real nut's boss under the flange — if it is ≈Ø9.5 the fix is facing off the flange, else source a plain round nut. (b) re-verify the listing before ordering; buying ONE rod (not ten pieces) is what makes shallow stock survivable. *Screw price is now settled: $41.45/600 mm, $50.42/800 mm, verified 2026-08-10* |
