...
| **TRRS M-M patch** (adapter → chassis socket) | Tensility **10-02155**: 3.5 mm 4C plug-to-plug, 1830 mm, 28 AWG shielded. **Overmould Ø6.1 × 14, barrel Ø3.5 × 14, cable Ø3.8** — all four off the drawing, and `leg_trrs` is built to them | 1 | **$3.73** [v] | [DigiKey](https://www.digikey.com/en/products/detail/tensility-international-corp/10-02155/7606602) · [Drawing](https://tensility.s3.us-west-2.amazonaws.com/uploads/pdffiles/10-02155.pdf) | the SHORT hop the leg's signal makes OUTSIDE the shell: one end is captive in the body adapter's bore (SPRING-FLOATED, so the blind mate seats wherever the latch stops), the other plugs UP into the adapter board's jack on the instrument's -Z UNDERSIDE, a hand's reach inboard of the leg — and unplugging THAT is what frees the leg. It leaves the body adapter through a CHANNEL in that adapter's top face: it comes up the bore +Z (the only way the rigid head gets in), folds over into the channel and runs out the -Y face, so the whole visible run is on the underside. **1830 is far more than the ~150 needed** — coil the excess in the cavity, or ask Tensility for the same assembly short (they build to order) |
