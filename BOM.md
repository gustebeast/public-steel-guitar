...
| **Motor retaining screw** | M4 × 0.7 × **6 mm** **button head** (ISO 7380) — the same **2.5 mm hex** as every other M4 on the instrument | 10 | commodity | commodity | ONE per motor, and the only fastener the motor bank uses. **The four M3 socket heads per motor are GONE (user, 2026-09-11):** the motors no longer bolt to the faceplate — each drops into a printed pocket (`motor_bank.pocket`) that stops it in X, ±Y and −Z, and this screw stops it lifting back out. It runs at **45° in the X–Z plane** through the pocket's +X post, so the driver comes straight down at 45° on every string — a +Y-facing head is unreachable at string 1 (the +Y rail) and a −Y one at strings 9–10 (the −Y rail). **SELF-TAPPING into PCTG, no insert:** a 45° insert pocket spans 8.5 mm in X and the post cannot be that thick before it reaches the neighbour's boss slot, while the Ø4.2 self-tap leaves 1.8 mm of wall either side (`motor_bank.SCREW_*`, asserted against the M4 spec's min_bite). It carries almost nothing: belt tension pulls each motor into that same post and gravity holds it down. |
