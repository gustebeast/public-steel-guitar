...
| **bus B, pedal segments** | **4 × 26 AWG** twisted (GND / +5 V / CAN_H / CAN_L), **~100 mm each, ×5**, PHR-8 both ends, ways 5–8 of one board to ways 1–4 of the next. **All five are the SAME length** — `foot_pedal.CABLE_LEN`, cut for the WIDEST pitch the bar can be built at — and the excess winds onto the cleat in the trough (`foot_pedal.CLEAT_*`, INSTALL_NOTES PB-1). That is the point: re-pitching the pedals means new bar pieces, and it must not also mean a new loom | ~2.5 bundled |
