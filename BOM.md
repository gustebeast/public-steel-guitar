...
| **Bridge axle** | Ø5 **g6/h6 precision shaft**, **107.9 mm** (e.g. hardened ground shafting) | 1 | [McMaster 5 mm shafts](https://www.mcmaster.com/products/linear-shafts/) | ~$3 [m] | NOT an m6 dowel — m6 is press-fit in the bearing bore; the shaft must SLIDE through all 10 bearings + 11 comb fingers + both arms in one pass, so it can carry no shoulder and nothing can be fitted to it from the side. **Installed from +Y, retained with NO fastener:** the −Y arm's bore is blind (that 1.6 wall is the −Y stop) and the OPTICAL STRIP is the +Y stop — its +X head turns over the endplate 0.75 outboard of the arm face the shaft ends flush with, and its underside sits 2.34 **below** the shaft's crown, so once the board is screwed down the shaft cannot travel +Y without driving its crown into FR4. **Length is not free** — it is the two stops, y −53.15 to +54.75 (dimensions.BRIDGE_AXLE_Y0/Y1). Order: bearings + fingers aligned → shaft in → board on |
| **Guide rod** | **Ø3 g6 precision shaft, ~26 mm** — the SAME stock as the bridge axle | 10 | [McMaster 3 mm shafts](https://www.mcmaster.com/products/linear-shafts/) | ~$0.5 ea [m] | Anti-rotation, through the H-nut's **−X ear**. **Ø3, not the old Ø2.5 dowel, and the reason is slop not strength:** the ear's hole is the nut's own Ø3, so a Ø2.5 rod leaves 0.5 mm of play — the nut rotates 38 mrad and the string walks 0.25 mm. Ø3 g6 leaves 0.01 mm and 0.8 mrad, fifty times better, and is 2.1× stiffer besides. It also **merges a BOM line**: same shaft as the bridge axle, just ten more pieces. **Mounting inverted (user):** it is pressed into the endplate's slab ABOVE and hangs DOWN, cantilevered. The old bottom socket is impossible now — the drive relief and the nut's sweep between them take out every scrap of endplate below the room at this X — and the top is also the end that prints, since the slab is a straight −X extension of solid cap. **Press fit**, because with that much socket any clearance is amplified over the rod's ~15 mm reach; bending is a non-issue at 0.016 mm |
| **Nut break dowel + clamp anvil** | Ø2 × 4 mm steel dowel (52100) | 20 | [McMaster 91595A018](https://www.mcmaster.com/91595A018/) | $12.70 / pack [m] | Two per string now, same part number. **BREAK DOWEL** ×10: the gauged break pins (the scale "0"), dropped into their slots from above. **ANVIL** ×10: a second dowel under the tail at the clamp, so the set screw pinches the string onto STEEL rather than onto the plastic floor. The anvil used to be optional — with the wrap capstan the clamp load fell far enough that it is cheap insurance rather than a necessity, but at one dowel per string it costs nothing to keep the pinch metal-on-metal |
| **Nut wrap rod (capstan)** | Ø5 **g6/h6 precision shaft, 69.6 mm** — the SAME stock as the bridge axle | 1 | [McMaster 5 mm shafts](https://www.mcmaster.com/products/linear-shafts/) | ~$2 [m] | **THE PART THAT MAKES THE CLAMP WORK.** Each string winds 1.5–3.5 turns around it before reaching its clamp, and the capstan (Euler-Eytelwein, T = T₀·e^−µθ, µ=0.15 steel-on-steel) divides the tension: **147 N → 5–36 N at the clamp**, i.e. 490 N of required clamp force → 18–119 N. The clamp was never the wrong mechanism; it was being asked for 490 N. **Not a new BOM line in substance** — it is the bridge axle's own Ø5 g6 shaft, so one shaft diameter now serves the bridge axle, this rod, and (at Ø3) the guide rods. Ø5 also bends the .070 at 26.2% outer-fibre strain vs Ø3's 37.2%, i.e. it sits about where a guitar tuner post sits. **Turns are per string and are a GEOMETRY result, not a preference** (`nut_block._turns`): a shared rod makes the coil climb ACROSS the strings, so each turn spends NUT_PITCH (6.5 mm). Every wrap marches −Y toward the thicker neighbour, which lands the fattest coil (.070, 6.53 mm of rod) in the free air outboard of the field — that is what buys it 3.5 turns and keeps every comb web ≥1.6 mm. Slides in from −Y through all ten webs at once, so it must be a precision shaft, not a dowel; the +Y bore is blind and that wall is its +Y stop |
...
10× 693ZZ (**~$30**), shaft collars **deleted** (−$25: the retainer is printed),
...
| 3 | **Tr5×1 H-flange nut — the model is built on GUESSED dimensions** | The fit problem is solved (the H cut is lane-ready at ~8.5 across flats, no lathe work), but the seller publishes **no geometry for the H version**. All six numbers now in `dimensions.py` — AF 8.5, flange 18.1 × 3.2, boss Ø8.0 × 6.6, ear holes Ø3 at ±6.5 — are extrapolated from the round disc's drawing. **$65 of nuts per instrument, $650 over a ten-instrument run, rides on them.** Stock is also thin across this size generally | **Order ONE first and measure it**, then update `dimensions.py` and rebuild. `NUT_FLANGE_L` and `NUT_HOLE_DX` are asserted in code (against the guide-rod sockets and the carriage's -X face), so a bad guess fails the build loudly. Watch `NUT_H` too: it feeds the nut-sweep slot's depth and the pulley-clearance assert. *Screw price settled: $10.97 for 2×350 mm, verified 2026-08-11* |
