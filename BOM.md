...
| **Ø5 precision shaft** | Ø5 **g6/h6 ground shafting** — bridge axle **100 mm** ×1 (`BRIDGE_AXLE_L` — a PURCHASED length now; the endplate arms derive from it, not the other way round. Stops: the −Y blind end wall and the optical strip), plus the knee-lever and foot-pedal axles | ~1 m stock | [McMaster linear shafts](https://www.mcmaster.com/products/linear-shafts/) | ~$3 [m] ⚠ | **Was Ø3 — the 695ZZ move took every axle to Ø5.** NOT an m6 dowel: m6 is press-fit in a 695ZZ bore, and the shaft must SLIDE through all 10 bearings + 9 comb fingers + both arms. NO GLUE: the −Y arm's bore is blind (the 1.6 wall is the −Y stop) and the optical strip's head is the +Y stop, so NO grub — the M2 that used to retain it is gone. ⚠ **Not yet re-priced or cut-listed at Ø5** — the per-axle lengths for the 6 levers + 5 pedals still need totalling before ordering |
...
| **M4 hold-down screw** | M4 × 0.7, 18 mm, 18-8 SS **button head** (ISO 7380, 2.5 mm hex) — a plain machine screw, NOT a Torx plastic thread-former | 1 | [McMaster 92095A-series](https://www.mcmaster.com/92095A192/) | ~$12 / pack [m] | the single +Z screw locking the merged keyhead nut-block endplate down — up from the floor bottom, forming its own thread in a Ø3.6 pilot in the PETG-GF boss, exactly as the leg lock screws do (the rest of the body is held by joinery). **Confirm ×18 is stocked at purchase** (16 and 20 are the common neighbours; the pilot depth decides which) |
| **M4 × 35 button screw** | M4 × 0.7, 35 mm, 18-8 SS button head (ISO 7380, 2.5 mm hex) | 14 | [McMaster 92095A-series](https://www.mcmaster.com/92095A192/) | ~$12 / pack [m] | 10 belt-tensioner draw screws (one per string, `belt_tensioner.SCREW_L`) + 4 chassis Y-retention shear pins (one per leg corner, down the rail web into the stub's inboard ridge; the Ø7.6 head sits in the Ø8.4 well). NEW row — both were already in the model with no BOM line. **Confirm the ×35 length suffix at purchase.** |
| **NEMA17 motor screw** | M3 × 0.5 × **10 mm** **socket head cap** (ISO 4762) — takes the same **2.5 mm hex** as every M4 button head; M3 itself is fixed by the motors' tapped holes | 40 | [McMaster](https://www.mcmaster.com/) | commodity | 4 per SERVO42D × 10. Threads the motor's own holes, so no insert — the one-insert-SKU rule still holds. **Length derived:** through the 6.4 motor plate (`motor_bank.PLATE_T`, no counterbore) leaves **3.6 of thread** in the motor (~1.2 d, fine in aluminium). ×12 would need 5.6 of tapped depth, more than a NEMA17 face usually has — it would bottom out and leave the motor loose. **⚠ The motor's tap depth is not measured** — check one SERVO42D before ordering. *(Replaces a generic 'Fasteners: M3, M2 (belt clamps)' line; belt_clamp is retired — the M4 × 35 row above took its job.)* |
