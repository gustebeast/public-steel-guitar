...
| **M4 × 35 button screw** | M4 × 0.7, 35 mm, 18-8 SS button head (ISO 7380, 2.5 mm hex) | 18 | [McMaster 92095A-series](https://www.mcmaster.com/92095A192/) | ~$12 / pack [m] | 10 belt-tensioner draw screws (one per string, `belt_tensioner.SCREW_L`) + 4 chassis Y-retention shear pins (one per leg corner, down the rail web into the stub's inboard ridge; the Ø7.6 head sits in the Ø8.4 well). **+ THE 4 LEG LOCK PINS** (`legs.lock_pin_joint`, one per corner, 2026-09-17): each threads a heat-set insert in its own ENDPLATE -- in the wall the deleted straight tongue used to hollow out -- and carries on through a Ø4.4 clearance hole in the chassis floor into the foot's TENONS in the bottom grid, which it pins. That is the whole leg retention now. They were M4 × 12 into an insert in the CHASSIS, which forced the floor to keep a mortise-free zone for the pocket. **35 is the only stocked length that lands in a tenon at every corner:** the +X/+Y foot gave up its two outboard tenons to the string access channels, so its nearest one is 36.4 in from the end face. At the other three the screw runs clean through two tenons and finishes in the third. Asserted in legs.py against the real grid, per corner. NEW row — both original uses were already in the model with no BOM line. **Confirm the ×35 length suffix at purchase.** |
